  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-20 pt-2 font-body">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-4">
            <h1 className="text-3xl lg:text-4xl font-heading font-extrabold text-neutral-dark tracking-tight">
              Order {order.orderNumber}
            </h1>
            <span className={cn(
              "px-3 py-1 text-[10px] font-extrabold rounded-full uppercase tracking-widest border",
          ...
          <div className="flex items-center gap-3 text-sm text-gray-400">
             <span className="font-medium">Placed on {new Date(order.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })} at 14:32</span>
             <span className="w-1 h-1 rounded-full bg-gray-200" />
             <span className="font-bold text-gray-500">2 Items Total</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
           ...
           <Card className="space-y-6 overflow-hidden" padding="none">
             <div className="px-6 py-5 border-b border-gray-50 bg-neutral-light/30 flex items-center justify-between">
                <h2 className="text-sm font-heading font-extrabold text-neutral-dark flex items-center gap-2 uppercase tracking-wider">
                  Order Items
                </h2>
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">SKU Based Fulfillment</span>
             </div>
               ...
               {order.items.map((item, idx) => (
                 <div key={idx} className="p-6 flex items-center gap-6 group hover:bg-neutral-light/20 transition-colors">
                   ...
                   <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] font-extrabold text-primary bg-primary/5 px-2 py-0.5 rounded-md uppercase tracking-tighter">
                          {item.variant}
                        </span>
                   ...
                   <div className="text-right">
                      <p className="text-sm font-extrabold text-neutral-dark font-mono italic">€{item.price.toFixed(2)}</p>
             ...
             <div className="bg-neutral-light/20 p-8 border-t border-gray-50">
                <div className="max-w-[300px] ml-auto space-y-3">
                   ...
                   <div className="flex justify-between items-center pt-1">
                      <span className="text-xs font-heading font-extrabold text-neutral-dark uppercase tracking-widest text-primary">Total Amount</span>
                      <span className="text-xl font-heading font-extrabold text-neutral-dark font-mono">€{order.total.toFixed(2)}</span>
                   </div>
                </div>
             </div>
           </Card>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="space-y-4" padding="md" shadow="sm">
                 <h3 className="text-[11px] font-heading font-extrabold text-[#1A1A2E]/60 uppercase tracking-widest flex items-center gap-2">
                   <CreditCard size={14} className="text-primary" />
                   Payment Details
                 </h3>
                 <div className="space-y-3 bg-neutral-light/50 p-4 rounded-xl border border-gray-100">
                    <div className="flex justify-between items-center">
                       <span className="text-[11px] font-bold text-gray-400 uppercase tracking-tight italic">Transaction ID</span>
                       <span className="text-[11px] font-bold text-neutral-dark font-mono uppercase">ch_2Nf82mE...</span>
                    </div>
                    <div className="flex justify-between items-center">
                       <span className="text-[11px] font-bold text-gray-400 uppercase tracking-tight italic">Status</span>
                       <span className="px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[10px] font-extrabold uppercase border border-emerald-100">Paid In Full</span>
                    </div>
                 </div>
              </Card>
              <Card className="space-y-4" padding="md" shadow="sm">
        ...
        <div className="space-y-8">
           ...
           <Card className="space-y-6" padding="md" shadow="sm">
              ...
              <div className="flex items-center gap-4">
                 ...
                 <div>
                    ...
                    <p className="text-xs text-gray-400 font-medium">Customer ID: CUST-8271</p>
                 </div>
              </div>
              ...
              <div className="space-y-4">
                 <h4 className="text-[10px] font-heading font-extrabold text-gray-400 uppercase tracking-widest">Delivery Address</h4>
           ...
           <Card className="space-y-6" padding="md" shadow="sm">
              ...
              <div className="space-y-6 pl-2">
                 {[
                   { status: "Delivered", time: "2h ago", desc: "Package left at reception.", active: true },
                   { status: "In Transit", time: "5h ago", desc: "Arrived at London Depot.", active: true },
                   { status: "Shipped", time: "8h ago", desc: "Picked up by DHL Express.", active: true },
                   { status: "Confirmed", time: "1d ago", desc: "Payment received via Stripe.", active: true },
                 ].map((event, i) => (
      ...
      <Modal
        ...
        title="Fulfill & Generate Waybill"
        maxWidth="md"
        footer={(
          <div className="flex justify-between items-center w-full">
            <div className="flex items-center gap-2 text-primary">
               <AlertCircle size={14} />
               <span className="text-[11px] font-bold">DHL API Integrated</span>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setIsFulfilling(false)} className="px-5 py-2 text-xs font-bold text-gray-400">Cancel</button>
      ...
      >
        <div className="space-y-6 pt-2">
           <div className="p-4 bg-neutral-light/50 rounded-2xl border border-gray-100 space-y-4">
              <div className="flex justify-between items-center">
                 <p className="text-[11px] font-heading font-extrabold text-[#1A1A2E]/60 uppercase tracking-widest">Weight Est.</p>
                 <span className="text-xs font-bold text-neutral-dark">1.25 kg</span>
              </div>
              <div className="flex justify-between items-center">
                 <p className="text-[11px] font-heading font-extrabold text-[#1A1A2E]/60 uppercase tracking-widest">Courier Service</p>
                 <span className="text-xs font-bold text-primary bg-primary/5 px-3 py-1 rounded-lg">DHL PRO-EXP 24H</span>
              </div>
           </div>

           <div className="space-y-3">
              <label className="text-[11px] font-heading font-extrabold text-gray-400 uppercase tracking-widest ml-1">Internal Note (Optional)</label>
              <textarea 
                ...
                placeholder="Added a free cap as loyalty bonus..."
              />
           </div>
           ...
           <div className="bg-primary/5 p-4 rounded-xl border border-primary/10 flex gap-3 text-primary">
              ...
              <p className="text-[11px] font-bold leading-normal">
                Generating waybill will automatically notify the customer via email and register the parcel with our local DHL office.
              </p>
           </div>
        </div>
      </Modal>
