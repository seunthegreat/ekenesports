...
export function PasswordStrength({ password = "" }: PasswordStrengthProps) {
  ...
  const requirements = useMemo(() => [
    { label: t("req_length"), test: (p: string) => p.length >= 8 },
    { label: t("req_upper"), test: (p: string) => /[A-Z]/.test(p) },
    { label: t("req_lower"), test: (p: string) => /[a-z]/.test(p) },
    { label: t("req_number"), test: (p: string) => /\d/.test(p) },
    { label: t("req_special"), test: (p: string) => /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]/.test(p) },
  ], [t]);
          ...
          return (
            <li
              ...
              className={cn(
                ...
                met ? "text-primary font-medium" : "text-neutral-dark/40"
              )}
            >
              {met ? (
